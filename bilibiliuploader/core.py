    ...
    with open(local_file_name, 'rb') as f:
        for chunk_id in range(0, chunk_total_num):
            ...
            for i in range(0, max_retry):
                if upload_chunk(upload_url, server_file_name, os.path.basename(local_file_name), chunk_data, CHUNK_SIZE, chunk_id,
                                chunk_total_num):
                    chunk_success = True
                    break
...
def upload(access_token,
    ...
    for video_part in parts:
        print("upload {}".format(video_part.title))
        status, server_file_name = upload_video_part(access_token, sid, mid, video_part, max_retry)
